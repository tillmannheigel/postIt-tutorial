...
Möglich wird dies mit der Instanzmethode \lstinline[basicstyle=\ttfamily]{getStateChange()}, welche die Konstante \lstinline[basicstyle=\ttfamily]{SELECTED} zurückgibt, falls ein Button angeklickt ist. Dies kann für die anderen Buttons analog umgesetzt werden. In unserem Post-It speichern wir den aktuellen Zustand der Buttons in boolschen Variablen, um später darauf zugreifen zu können.
...
Dafür erstellen wir uns zunächst Getter-Methoden für unsere drei Button Boolean-Felder und das Schriftart String-Feld. In den meisten IDE's geht das mit ein paar Menü-klicks (einfach mal Googlen. In Eclipse geht das recht einfach mit einem Klick auf \textit{Source} -> \textit{Generate Getters and Setters}). Das Ergebnis sollten folgende vier Getter-Methoden sein:
...
Damit haben wir die Schnittstellen auf der Seite der Klasse \lstinline[basicstyle=\ttfamily]{ToolbarPanel} gebaut. Nun wechseln wir in die Klasse \lstinline[basicstyle=\ttfamily]{NotesTextpane}. 
